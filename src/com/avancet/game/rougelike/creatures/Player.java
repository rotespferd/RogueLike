...
public class Player extends Creatures {
	...
	public Player(int heightCord, int widthCord) {
		...
		GameLogger.getLogger().info(
				"Init the player with name=" + this.getName());
	}
	

	protected void paintObject(Graphics g) {
		// g.drawLine(w * size + 15, h * size + 17, w * size + size, h * size +
		// size);
		g.drawImage(TextureStorage.getTexture("Player"), this.getCordW() * size
				+ 15, this.getCordH() * size + 15, null);
	}

	public void movePlayer(String direction, GameMap map) {
		if ("UP".equals(direction))
			validateAndMovePlayer(map, this.getCordW(), this.getCordH() - 1);
		else if ("DOWN".equals(direction))
			validateAndMovePlayer(map, this.getCordW(), this.getCordH() + 1);
		else if ("RIGHT".equals(direction))
			validateAndMovePlayer(map, this.getCordW() + 1, this.getCordH());
	...
	private void validateAndMovePlayer(GameMap map, int newW, int newH) {
		...
		if ((map.getGameWorld().getWorldElement(newH, newW)).isSolid())
			return;
